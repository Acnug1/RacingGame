...
public class AIController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Vector3.Distance(_selfTransform.position, current.position) < Random.Range(5f, 6f) && Vector3.Distance(_selfTransform.position, current.position) > Random.Range(2f, 3f) && _waypoints.Length != 1) // немного оттормаживаемся заранее на опеределенной дистанции до цели
        {
